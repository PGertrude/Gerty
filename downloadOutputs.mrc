...
alias lastNdays {
  ...
  if ($gettok($hget(%thread,command),1,32) == Last || $tracks($hget(%thread,command))) {
    if (%r1 == 0) { $hget(%thread,out)  $+ $hget(%thread,nick) Skills  $+ $hget(%thread,command) $+ : 07Overall 03+0 exp; | goto unset }
    var %cmb = $sort(Attack %2 %r2 Defence %3 %r3 Strength %4 %r4 Hitpoints %5 %r5 Range %6 %r6 Pray %7 %r7 Mage %8 %r8 Summon %25 %r25)
    if (%cmb) { var %out1 = $hget(%thread,out)  $+ $hget(%thread,nick) Combat  $+ $hget(%thread,command) $+ : %cmb }
    var %others = $sort(Cook %9 %r9 Woodcut %10 %r10 Fletching %11 %r11 Fishing %12 %r12 Firemake %13 %r13 Crafting %14 %r14 Smithing %15 %r15 Mine %16 %r16 Herblore %17 %r17 Agility %18 %r18 Thieve %19 %r19 Slayer %20 %r20 Farming %21 %r21 Runecraft %22 %r22 Hunter %23 %r23 Construct %24 %r24)
    if (%others) { var %out2 = $hget(%thread,out)  $+ $hget(%thread,nick) Others  $+ $hget(%thread,command) $+ : %others }
    $hget(%thread,out)  $+ $hget(%thread,nick) Skills  $+ $hget(%thread,command) $+ : 07Overall $iif(%total > 0,[+ $+ %total $+ ]) 03+ $+ $iif(%r1 > 1000,$bytes($round($calc($v1 /1000),0),db) $+ k,$bytes($v1,db)) exp;
    %out1
    %out2
  }
  else {
    ...
    if (%others) { var %out2 = $hget(%thread,out)  $+ $hget(%thread,nick) Others  $+ $hget(%thread,command) $+ : %others }
    $hget(%thread,out)  $+ $hget(%thread,nick) Skills  $+ $hget(%thread,command) $+ : 07Overall $iif(%total > 0,[+ $+ %total $+ ]) 03+ $+ $iif($calc(%r1 - %r26) > 1000,$bytes($round($calc($v1 /1000),0),db) $+ k,$bytes($v1,db)) exp;
    %out1
    %out2
  }
  :unset
