...
if ($ltracks($right($1,-1)) == NoMatch) { halt }
var %class34 = $+($r(0,9),$r(0,9),$r(0,9),$r(0,9),$r(0,9),$r(0,9))
if ($2) {
...
hadd -m $+(n,%class34) out .notice $nick
hadd -m $+(n,%class34) command $ltracks($right($1,-1))
hadd -m $+(n,%class34) nick %nick
sockopen $+(yesterday.,$+(n,%class34)) hiscore.runescape.com 80
}
on *:TEXT:*:?: {
if ($left($1,1) != . && $left($1,1) != ! && $left($1,1) != @) { halt }
if ($ltracks($right($1,-1)) == NoMatch) { halt }
...
if ($2) { var %nick = $regsubex($rsn($replace($2-,$chr(32),_)),/\W/g,_) }
if (!$2) { var %nick = $regsubex($rsn($nick),/\W/g,_) }
...
hadd -m $+(a,%class) command $ltracks($right($1,-1))
hadd -m $+(a,%class) nick $caps(%nick)
if ($tracked(%nick) == tracked) { sockopen $+(yesterday.a,%class) hiscore.runescape.com 80 }
...
if ($ltracks($right($1,-1)) == yesterday) {
var %time = $calc(%today + 86400)
var %time2 = %today
}
if ($ltracks($right($1,-1)) == lastweek) {
var %time = $calc(%week +604800)
var %time2 = %week
}
if ($ltracks($right($1,-1)) == lastmonth) {
set %lastmonth $replace($date(mmm),Jan,2419200,Feb,2678400,Mar,2592000,Apr,2678400,May,2592000,Jun,2678400,Jul,2678400,Aug,2592000,Sep,2678400,Oct,2592000,Nov,2678400,Dec,2678400)
var %time = $calc(%month + %lastmonth )
var %time2 = %month
}
if ($ltracks($right($1,-1)) == lastyear) {
var %time = $calc(31536000 + %year)
var %time2 = %year
}
hadd -m $+(a,%class) time %time
hadd -m $+(a,%class) time2 %time2
sockopen $+(trackyes.a,%class) www.rscript.org 80
}
}
on *:sockopen:yesterday.*: {
...
inc %row 1
if (%row == 30) { closeyesterday }
}
}
alias closeyesterday {
...
var %x = 25
while (%x) {
set %g [ $+ [ %x ] ] $gettok($readini($hget($gettok($sockname,2,46),command) $+ .ini,$hget($gettok($sockname,2,46),nick),$statnum(%x)),3,44)
dec %x
}
var %x = 25
while (%x) {
set %rg [ $+ [ %x ] ] $gettok($readini($hget($gettok($sockname,2,46),command) $+ .ini,$hget($gettok($sockname,2,46),nick),$statnum(%x)),1,44)
dec %x
}
