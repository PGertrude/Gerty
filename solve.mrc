...
if (!%answer && $len($1-) > 2) {
.notice $nick $mid($calcparse($right($1-,-1)),1,10) 12=> 07{Invalid Parameters}
}
}
on $*:text:/^[=`](-*[0-9\.sctapelx\x28]) */Si:?: {
%answer = $regsubex($strictcalc($right($1-,-1)),/(\d+(\.\d+)?)/g,$bytes(\1,db))
...
if (!%answer && $len($2-) > 1) {
%saystyle $mid($calcparse($2-),1,10) 12=> 07{Invalid Parameters}
}
}
}
on *:TEXT:*:?: {
...
%eq = $regsubex(%eq,/(sin|asin|acos|atan|cos|tan|log|sqrt|lvl|l)(x+)/g,\1 $+ $regsubex(s,\2,/(x)/g,$+($chr(40),x,$chr(41))))
%eq = $regsubex(%eq,/(sin|asin|acos|atan|cos|tan|log|sqrt|lvl|l)(e+)/g,\1 $+ $regsubex(s,\2,/(e)/g,$+($chr(40),e,$chr(41))))
%eq = $regsubex(%eq,/(sin|asin|acos|atan|cos|tan|log|sqrt|lvl|l)(z+)/g,\1 $+ $regsubex(s,\2,/(z)/g,$+($chr(40),z,$chr(41))))
%eq = $regsubex(%eq,/(sin|asin|acos|atan|cos|tan|log|sqrt|lvl|l)(p+)/g,\1 $+ $regsubex(s,\2,/(p)/g,$+($chr(40),p,$chr(41))))
%eq = $regsubex(%eq,/(sin|asin|acos|atan|cos|tan|log|sqrt|lvl|l)(\d+)/g,\1 $+ $chr(40) $+ \2 $+ $chr(41))
