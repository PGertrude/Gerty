...
;nick given and time
if ($3 && $left($3,1) != @ && @ isin $4-) {
hadd -m $+(a,%class) nick $caps($regsubex($rsn($left($gettok($3-,1,64),-1)),/\W/g,_))
hadd -m $+(a,%class) skill $scores($2)
hadd -m $+(a,%class) time $duration($gettok($3-,2,64))
hadd -m $+(a,%class) command $duration($duration($gettok($3-,2,64)))
}
;no nick, but time
if ($3 && $left($3,1) == @) {
hadd -m $+(a,%class) nick $caps($regsubex($rsn($nick),/\W/g,_))
hadd -m $+(a,%class) skill $scores($2)
hadd -m $+(a,%class) time $duration($right($3-,-1))
hadd -m $+(a,%class) command $duration($duration($right($3-,-1)))
}
;nick given and no time
if ($3 && @ !isin $3-) {
hadd -m $+(a,%class) nick $caps($regsubex($rsn($3-),/\W/g,_))
hadd -m $+(a,%class) skill $scores($2)
hadd -m $+(a,%class) time %today
hadd -m $+(a,%class) command Today
}
;no nick/time
if (!$3) {
hadd -m $+(a,%class) nick $caps($regsubex($rsn($nick),/\W/g,_))
...
if ($lookups($2) != overall) {
sockopen $+(track.a,%class) www.rscript.org 80
}
if ($lookups($2) == overall) {
sockopen $+(trackoas.,$+(a,%class)) www.rscript.org 80
}
}
;skill not specified
if ($lookups($2) == nomatch) {
;nick given and time
if ($2 && $left($2,1) != @ && @ isin $2-) {
hadd -m $+(a,%class) nick $caps($regsubex($rsn($left($gettok($2-,1,64),-1)),/\W/g,_))
hadd -m $+(a,%class) skill all
hadd -m $+(a,%class) time $duration($gettok($2-,2,64))
hadd -m $+(a,%class) command $duration($duration($gettok($2-,2,64)))
}
;no nick, but time
if ($2 && $left($2,1) == @) {
hadd -m $+(a,%class) nick $caps($regsubex($rsn($nick),/\W/g,_))
hadd -m $+(a,%class) skill all
hadd -m $+(a,%class) time $duration($right($2-,-1))
hadd -m $+(a,%class) command $duration($duration($right($2-,-1)))
}
;nick given and no time
if ($2 && @ !isin $2-) {
hadd -m $+(a,%class) nick $caps($regsubex($rsn($2-),/\W/g,_))
hadd -m $+(a,%class) skill all
hadd -m $+(a,%class) time %today
hadd -m $+(a,%class) command Today
}
...
sockopen $+(trackall.a,%class) www.rscript.org 80
}
}
;###########################
;#######Single Skill########
...
hadd -m $+(a,%class) time %time
hadd -m $+(a,%class) command Last $regsubex($duration($duration($regml(2) $+ $regml(3))),/(\d+)([A-Za-z]+)/g,\1 \2)
...
hadd -m $+(a,%class) nick %nick
hadd -m $+(a,%class) time %time
hadd -m $+(a,%class) time2 $calc(%time - 86400)
hadd -m $+(a,%class) command $regsubex($duration($calc(%time - %today)),/(\d+)([A-Za-z]+)/g,\1 \2) ago
hadd -m $+(a,%class) skill $skills($2)
hadd -m $+(a,%class) out $saystyle($left($1,1),$nick,$chan)
sockopen $+(trackyes.a,%class) www.rscript.org 80
}
...
/*
var %todaylv = $calc($oatoday(attack) + $oatoday(strength) + $oatoday(defence) + $oatoday(hitpoints) + $oatoday(ranged) + $oatoday(prayer) + $oatoday(magic) + $oatoday(cooking) + $oatoday(woodcutting) + $oatoday(fletching) + $oatoday(fishing) + $oatoday(firemaking) + $oatoday(crafting) + $oatoday(smithing) + $oatoday(mining) + $oatoday(herblore) + $oatoday(agility) + $oatoday(thieving) + $oatoday(slayer) + $oatoday(farming) + $oatoday(runecraft) + $oatoday(hunter) + $oatoday(construction) + $oatoday(summoning))
var %weeklv = $calc($oaweek(attack) + $oaweek(strength) + $oaweek(defence) + $oaweek(hitpoints) + $oaweek(ranged) + $oaweek(prayer) + $oaweek(magic) + $oaweek(cooking) + $oaweek(woodcutting) + $oaweek(fletching) + $oaweek(fishing) + $oaweek(firemaking) + $oaweek(crafting) + $oaweek(smithing) + $oaweek(mining) + $oaweek(herblore) + $oaweek(agility) + $oaweek(thieving) + $oaweek(slayer) + $oaweek(farming) + $oaweek(runecraft) + $oaweek(hunter) + $oaweek(construction) + $oaweek(summoning))
var %monthlv = $calc($oamonth(attack) + $oamonth(strength) + $oamonth(defence) + $oamonth(hitpoints) + $oamonth(ranged) + $oamonth(prayer) + $oamonth(magic) + $oamonth(cooking) + $oamonth(woodcutting) + $oamonth(fletching) + $oamonth(fishing) + $oamonth(firemaking) + $oamonth(crafting) + $oamonth(smithing) + $oamonth(mining) + $oamonth(herblore) + $oamonth(agility) + $oamonth(thieving) + $oamonth(slayer) + $oamonth(farming) + $oamonth(runecraft) + $oamonth(hunter) + $oamonth(construction) + $oamonth(summoning))
var %yearlv = $calc($oayear(attack) + $oayear(strength) + $oayear(defence) + $oayear(hitpoints) + $oayear(ranged) + $oayear(prayer) + $oayear(magic) + $oayear(cooking) + $oayear(woodcutting) + $oayear(fletching) + $oayear(fishing) + $oayear(firemaking) + $oayear(crafting) + $oayear(smithing) + $oayear(mining) + $oayear(herblore) + $oayear(agility) + $oayear(thieving) + $oayear(slayer) + $oayear(farming) + $oayear(runecraft) + $oayear(hunter) + $oayear(construction) + $oayear(summoning))
*/
