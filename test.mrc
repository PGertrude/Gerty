>start<|test.mrc|Calculator alias'|1.6|rs
alias calcreg {
  ...
  return $replace(%string,~,3.141593,e,2.718281,p,$hget($nick,p))
}
...
alias _checkCalc {
  ...
  if ($_checkFunct(%string)) { return false Unknown function:07 $v1 }
...
alias _checkFunct {
  var %string = $remove($1-,$chr(44)), %x = 1
