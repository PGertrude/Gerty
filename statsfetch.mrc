...
noop $download.break(stats %saystyle $replace(%skill,$chr(32),_) %skillid %nick %param %goal %morethan %lessthan %socket %presetgoal %presetparam,%socket,%url)
:unset
unset %*
}
alias stats {
var %saystyle = $1 $2, %skill = $replace($3,_,$chr(32)), %skillid = $4, %nick = $5, %param = $replace($6,~,$chr(32)), %goal = $7, %morethan = $8, %lessthan = $9, %socket = $10, %presetgoal = $11, %presetparam = $12
.tokenize 10 $distribute($13)
...
if ($calculate(%param) isnum) {
%param = $calculate(%param)
...
var %socket = $1 $+ 2, %nick = $2, %skill = $3,%saystyle = $4-, %today, %week, %month, %year
...
%month = $calc(($date(dd) - 1) * 86400 - 25200 + %today )
}
%year = $calc($ctime($date $time) - $ctime(January 1 $date(yyyy) 07:00:00))
var %url = http://www.rscript.org/lookup.php?type=track&user= $+ %nick $+ &skill= $+ $calc($statnum(%skill) - 1) $+ &time= $+ %today $+ , $+ %week $+ , $+ %month $+ , $+ %year
noop $download.break(rscript.singleskillreply %saystyle %skill %nick,%socket,%url)
}
alias rscript.singleminigame {
var %socket = $1 $+ 2, %nick = $2, %skill = $replace($3,_,$chr(32)),%saystyle = $4-, %today, %week, %month, %year
if ($duration($time) >= 25200) {
%today = $calc($duration($time) - 25200)
}
if ($duration($time) < 25200) {
...
%month = $calc(($gettok($date,1,47) - 1) * 86400 - 25200 + %today )
}
%year = $calc($ctime($date $time) - $ctime(January 1 $date(yyyy) 07:00:00))
